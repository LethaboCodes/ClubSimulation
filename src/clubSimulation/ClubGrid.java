...
public class ClubGrid {
    ...
    synchronized (entrance) {
				...
				while (!entrance.get(myLocation.getID())) {
					entrance.wait(); // Wait until the entrance is available
			  }
        while (counter.overCapacity()) { // Wait until there's space available
            System.out.println("Club is at maximum capacity. " + Thread.currentThread().getName() + " is waiting...");
            entrance.wait();
